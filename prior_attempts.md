...
**Analytic closure on DST-concave (run 20260606-26-auto, UNIRANK review).** On DST-concave, channel 2 is deterministic given survival: every alive-at-`t` trajectory has identical `M^2_t = -(1-γ^{t+1})/(1-γ)`. As a consequence: (a) PARGRAD's bivariate weak-dominance count reduces exactly to the channel-1 strict-below CDF `F̂^-_t(M^1_t)` on the alive cohort (the `M^2_j ≤ M^2_t` clause is satisfied with equality for all alive `j`, so only channel-1 strictness contributes); (b) COPDEV's per-channel rank-disagreement `|F̂^1_t - F̂^2_t|` reduces to `|F̂^1_t - 1|` (since `F̂^2_t` is a unit step at the constant alive-cohort value, giving `F̂^2_t(M^2_t) = 1` for every alive member). Both bivariate primitives collapse to unicriterial channel-1 statistics on this substrate. This means DST-concave cannot discriminate bivariate-rank mechanisms from unicriterial-rank mechanisms. Any empirical signal a bivariate-rank candidate shows on DST-concave is unicriterial signal in disguise. **A bivariate-rank probe must be tested on a substrate where channel 2 varies non-trivially across the alive cohort** (i.e., channel 2 is not deterministic given survival-to-`t`).
